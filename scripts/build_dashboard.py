...
def main():
    ...
    for stream in active:
        ...
        lines.append(f"| {stream['stream_id']} · {stream['title']} | {stream['assignment']} | {stream['stage']} | {plan} | {progress} |")
    lines.extend(["", "## Фабрики", "", "| Фабрика | Статус | Потоки | Репозиторий |", "|---|---|---:|---|"])
    for factory in factory_data["factories"]:
        lines.append(f"| {factory['name']} | {factory['status']} | {factory['active_streams']} | {factory['repository']} |")
    lines.extend(["", "## Последние события", "", "| Время | Событие | Было | Стало |", "|---|---|---|---|"])
    for event in events[-10:]:
        lines.append(f"| {event['event_time']} | {event['event_type']} | {event['from_assignment']} | {event['to_assignment']} |")
    lines.extend(["", "> Фактическое время запусков не вычисляется из расписания. Если телеметрия запуска отсутствует, значение остаётся неизвестным.", ""])
